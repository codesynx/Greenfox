...
export default function FavoritesScreen() {
  ...
  const loadFavorites = useCallback(async () => {
    try {
      setIsLoading(true);
      // Refresh the global favorites context
      await refreshFavorites();
      // Load detailed favorites for display
      ...
      <YStack paddingHorizontal="$4" paddingTop={insets.top + 32} paddingBottom="$4">
        ...
        {!isLoading && (
          <Text fontSize={14} color="rgba(255, 255, 255, 0.6)" marginTop="$2">
            {favorites.length} {t('favorites.savedProperties')}
          </Text>
        )}
      </YStack>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <YStack paddingHorizontal="$4" paddingBottom="$20">
            {favorites.map((favorite) => {
              const resort = favorite.resort;
              const price = resort.promo && resort.promoPrice ? resort.promoPrice : resort.basePrice;
              const image = resort.mainPhotoUrl || 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=800';

              return (
                <Pressable key={favorite.id} onPress={() => (navigation as any).navigate('Details', { propertyId: resort.id })}>
                  <XStack
                    marginBottom="$4"
                    borderRadius={16}
                    overflow="hidden"
